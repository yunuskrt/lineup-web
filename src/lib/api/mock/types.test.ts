...
describe('mockSquadSchema', () => {
  it('accepts a fixture squad', () => {
    expect(mockSquadSchema.safeParse(SQUAD).success).toBe(true);
  });

  it('rejects a squad that is not exactly eleven', () => {
    expect(mockSquadSchema.safeParse(SQUAD.slice(0, 10)).success).toBe(false);

    const twelve = [...SQUAD, { ...SQUAD[0], playerId: 'pl-12' }];
    expect(mockSquadSchema.safeParse(twelve).success).toBe(false);
  });

  it('rejects two players sharing a formation slot', () => {
    const clashing = SQUAD.map((entry, index) =>
      index === 1 ? { ...entry, slot: SQUAD[0].slot } : entry,
    );
  ...
  it('rejects a player with no aliases', () => {
    const aliasless = SQUAD.map((entry, index) =>
